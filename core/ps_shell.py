...
class PsCmd(object):
    ...
    def ops_ps_cmd(self, ps_name='all', op_type='cat'):
        ...
        self.f.write(
            '\n\n------------------{}操作进程开始--------------\n\n'.format(
                time_utils.get_curr_dt() +
                time_utils.get_curr_time()))
        if ps_name == 'all':
            ...
            if ret['res']:
                ...
                if op_type == 'start':
                    ...
                    if cd_ret['code'] == 0:
                        ...
                        cmd_utils.cmd_exec('mv %s %s_%s' % (
                            log_path, log_path, str(int(time.time()))))
                        os.chdir(cmd_path)  # 切换路径
                        start_ret = cmd_utils.cmd_exec(start_cmd)
                        cmd_utils.cmd_exec('sleep 10')
                        # print(start_ret)
                        log_ret = cmd_utils.cmd_exec(
                            'tail -30 {}'.format(log_path))
                        print(
                            Fore.RED +
                            Style.BRIGHT +
                            '-------------启动日志{}------------\n'.format(
                                ps_dict.get(item).get('run_log')) +
                            log_ret['res'] +
                            "\n")

                        self.f.write(
                            '-------------启动日志{}------------\n'.format(
                                ps_dict.get(item).get('run_log')) + log_ret['res'] + "\n")
                    else:
                        print(
                            Fore.RED + Style.BRIGHT + '进程[%s]所在路径[%s]不存在 ' %
